...
class NotifyGlobalDataWithLivedataActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_toFragmentA.setOnClickListener { supportFragmentManager.beginTransaction().replace(R.id.fl_container, FragmentA()).commit() }
        btn_toFragmentB.setOnClickListener { supportFragmentManager.beginTransaction().replace(R.id.fl_container, FragmentB()).commit() }
    }
}